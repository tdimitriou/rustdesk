...
pub static ref T: std::collections::HashMap<&'static str, &'static str> =
    [
        ...
        ("id_change_tip", "Only a-z, A-Z, 0-9, @, . (dot), - (dash) and _ (underscore) characters allowed. The first letter must be a-z, A-Z. @ is mandatory and must be followed by at least 3 letters or numbers. Length between 8 and 32."),
        ("id_rule_valid_format", "Matches custom ID rules"),
        ("change_id_uuid_mismatch", "This device does not match the ID record on the server (UUID or public key). Change ID must be done from the same installation that registered that ID, or use a new ID."),
